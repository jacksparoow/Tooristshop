...
console.log('update usern local storage',this.updatedUser);
      ...
      this.updateusersrv.get_projects_of_company(localStorage.getItem('user_name')).subscribe((data:Project[])=>{
        this.projects = data;
  ...
  confirmPassword(event: any){
   ...
   // console.log('p',this.user.password);
    if(this.updatedUser.passwordhash === event.target.value){
       this.cpy = true;
       this.cpn = false;
    }else{
     this.cpy = false;
  ...
  selectRole(){
    for(let i =0;i<this.roles.length;i++){
      if(this.selectedRole==this.roles[i].name){
        this.updatedUser.roleid = this.roles[i].id;
        break;
      }
    }
